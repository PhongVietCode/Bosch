...
{
    if (index > array_length) // Phong check ">="
    {
...
void * allocate_buffer(unsigned long size)
{
    ...
    unsigned char i; // Phong check unsign char
...
int deallocate_buffer(void * buffer)
{
    ...
    unsigned char i;
    for (i = 0; i < header->size; i++) // Phong check header->size
    {
...
int calculate_fingerprint(unsigned long uid, unsigned long counter, const char *text, unsigned int* output)
{
    ...
    (void)memcpy(&workspace[sizeof(uid) + sizeof(counter)], text, strlen(text)); // Phong check strlen
